 ...
 * 需要使用 namedThreadPool.execute(new NamedRunnable("taskName", runnable)) 来提交任务，而非 submit
...
public class DynamicThreadPool extends ThreadPoolExecutor {
    ...
    public void execute(@Nonnull Runnable r) {
        ...
        } else if (r instanceof NamedFutureTask<?> namedFutureTask) {
            NamedCallable<?> namedCallable = namedFutureTask.getNamedCallable();
            namedCallable.setSubmitTime(System.currentTimeMillis());
            super.execute(namedFutureTask);
        } else {
    ...
    public <T> Future<T> submit(Callable<T> task) {
        if (task instanceof NamedCallable<T> namedCallable) {
            NamedFutureTask<T> futureTask = new NamedFutureTask<>(namedCallable);
            execute(futureTask);
            return futureTask;
        } else {
    ...
    protected void beforeExecute(Thread t, Runnable r) {
        ...
        if (r instanceof NamedRunnable namedRunnable) {
            ...
            if (this.threadPoolConfig.getMonitor().isEnabled()) {
                ...
                log.info("Task {} waited {} ms before execution", taskName, waitTime);
                registerTaskWaitTime(taskName, waitTime);
            }
        } else {
            throw new RunnableNotSupportException();
    ...
    protected void afterExecute(Runnable r, Throwable t) {
        ...
        if (r instanceof NamedRunnable namedRunnable) {
            ...
            if (this.threadPoolConfig.getMonitor().isEnabled()) {
                ...
                log.debug("NamedRunnable Task {} executed in {} ms", taskName, executionTime);
            ...
            if (this.threadPoolConfig.getMonitor().isEnabled()) {
                ...
                log.info("NamedCallable Task {} executed in {} ms", taskName, executionTime);
                getCollector().ifPresent(metricsCollector -> recordTaskAfterFinish(metricsCollector, t, taskName, executionTime));
            }
        } else {
            throw new RunnableNotSupportException();
